        ...
        if(global.fullarr.game_bool[game_bool_index].game_bool == 'true' && global.fullarr.channels[kanal_index].channel_id == message.channel.id){
            ...
            if(global.fullarr.son_kelime_yazan[son_kelime_yazan_index].son_kelime_yazan == message.author.id){
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(`${message.author.username}, üst üste yazamazsınız başka birisinin yazdıktan sonra yazmalısınız.`)
                    .setColor("GREEN");
            ...
            if(kelime.match(reg)){
                const embed = new Discord.MessageEmbed()
                    .setAuthor(message.author.username, message.author.avatarURL(),"https://discord.gg/SqEhPJFmAE")
                    .setDescription(`**${kelime}**, geçersiz karakter içeriyor. Lütfen sadece türkçe karakter ve harf kullanın komutta bir yanlışlık yoktur TDK'dan çekmektedir.`)
                    .setColor("GREEN");
                message.channel.send(embed).then(del => del.delete({timeout:2000} , message.delete({timeout:2000})));
                return;                
            }
            if(global.fullarr.son_kelime[son_kelime_index].son_harf && global.fullarr.son_kelime[son_kelime_index].son_harf !== kelime[0]){
                ...
                const embed = new Discord.MessageEmbed()
                    .setAuthor(message.author.username, message.author.avatarURL(),"https://discord.gg/SqEhPJFmAE")
                    .setDescription(`**${kelime}**, bu kelime **${stngssonharf}** ile başlamıyor lütfen **${stngssonharf}** ile başlayan bir kelime yazın.`)
                    .setColor("GREEN");
                message.channel.send(embed).then(del => del.delete({timeout:2000} , message.delete({timeout:2000})));
                return;
            }
                        ...
                        fs.writeFile(settings_path, JSON.stringify(global.fullarr, null, 2), 'utf-8', function(err) {
                            ...
                            setTimeout(() => {
                                ...
                                fs.writeFile(settings_path, JSON.stringify(global.fullarr, null, 2), 'utf-8', function(err) {
                                    ...
                                    .setDescription(`Oyun solandırıldığı için **BOT** tarafından sıfırlandı ve aktif edildi. Herkese iyi eğlenceler dileriz.\nBaşlangıç kelimesi: **${sansli_isim}**.`)
                                    .setColor("GREEN");
                                    message.channel.send(embed);
                                });
                            }, 5000);
